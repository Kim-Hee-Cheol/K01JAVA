...
-RuntimeException클래스 계열로 NullPointerException이
대표적인 하위 클래스이다.
-main메소드에서 무시하기 위해 throws하더라도 오류가 발생되므로
반드시 예외처리를 해줘야 한다.
-예외발생 시 JVM은 해당 예외클래스를 인스턴스화해서 프로그램쪽으로
...
public class Ex02PreDefineException {
	...
	public static void main(String[] args) {
		...
		4] NullPointerException
			: 인스턴스 변수에 참조값이 없는 상태에서 조출하면
			발생되는 예외 즉 참조변수만 생성되고, 참조 할 객체가
			없는 상태에서 사용했을 때 발생하게된다.
		 */
		...
		5] ArithmeticException
			: 수학적 계산이 불가능한 경우 발생되는 원인
		 */
		System.out.println("### ArithmeticExcpetion ###");
		int result  = 10;
		try {
			result = result / 0;
